...
//YZZDataCapturer *myDataCapturer = new YZZDataCapturer;
bool calibratingLoc01=false;
bool calibratingLoc02=false;
bool detectingInput=false;
//YZZDataQueuer myDataQueuer;
//CrossCorr myCrossCorr;

//---buffers used for template matching---//
CircularBuffer *accelLoc01Buffer = new CircularBuffer(DATA_BUFF_STRUCT_SIZE);
CircularBuffer *micLoc01Buffer = new CircularBuffer(MIC_DATA_BUFF_SIZE);
CircularBuffer *accelLoc02Buffer = new CircularBuffer(DATA_BUFF_STRUCT_SIZE);
CircularBuffer *micLoc02Buffer = new CircularBuffer(MIC_DATA_BUFF_SIZE);
CircularBuffer *accelCaptureBuffer = new CircularBuffer(DATA_BUFF_STRUCT_SIZE);
CircularBuffer *micCaptureBuffer = new CircularBuffer(MIC_DATA_BUFF_SIZE);
