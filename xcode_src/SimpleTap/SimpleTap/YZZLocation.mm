...
-(void) clearAllFiles {
    NSLog(@"hey hey  %d", [self.SensorFileHandles count]);
    for (int i=0;i<TOTAL_NUMBER_OF_SENSORS;i++) {
    ...
    [self.SensorFileHandles removeAllObjects];
    NSLog(@"hey heyhey %d", [self.SensorFileHandles count]);
}
-(void)createFilesAndAttacheFileHandlesForAllBuffers {
    NSLog(@"heyasdgasdf  %d", [self.SensorFileHandles count]);
    ...
    NSLog(@"heyjdfjfdtrgas  %d", [self.SensorFileHandles count]);
}
...
//                NSLog(@"HERE HERE i=%d %f %@",i, linearBuffer[j], bufferString);
                if ([self.SensorFileHandles objectAtIndex:i]==NULL) {
                    assert(false);
                }
//                printf("hey hey\n");
                if ([self.myFileManager fileExistsAtPath:[self.SensorFilePaths objectAtIndex:i]]==true) {
//                    NSLog(@"%@", [self.SensorFilePaths objectAtIndex:i]);
                    //assert(false);
                }
//                NSLog(@"YOYO %d", [self.SensorFilePaths count]);
//                [[self.SensorFileHandles objectAtIndex:i] writeData:[@"hihi\n" dataUsingEncoding:NSUTF8StringEncoding]];
